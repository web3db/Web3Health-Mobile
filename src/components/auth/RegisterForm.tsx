...
/**
 * Lightweight schema: only fields required to create MST_User
 */
...
export default function RegisterForm() {
  ...
  const { user } = useUser();
  const router = useRouter();
  const { signOut } = useAuth(); // kept for parity

  const { name, email, clerkId, setField, reset } = useRegisterStore();

  // ---- ONE-SHOT Clerk → store hydration
  const didHydrateFromClerk = React.useRef(false);
  React.useEffect(() => {
    if (didHydrateFromClerk.current) return;
  ...
  const onSubmit = useCallback(async () => {
    try {
      ...
      const parsed = RegisterLiteSchema.parse({
        clerkId,
        email: email ?? null,
        name,
        roleId: 1,
  ...
  }, [clerkId, email, name, reset, router]);

  const disabled = submitting || !name?.trim() || !clerkId;
  ...
  return (
    ...
    >
      <View style={{ gap: 6 }}>
        ...
        >
          Welcome to Web3Health
        </Text>

        <Text style={{ color: c.text.muted, lineHeight: 20 }}>
        ...
        >
          <Text
            style={{ color: c.text.secondary, fontSize: 12, fontWeight: "600" }}
          >
            Quick setup
          </Text>
        </View>
      ...
      <View style={{ gap: 8 }}>
        <Text style={{ color: c.text.primary, fontWeight: "700" }}>
          Your name <Text style={{ color: c.primary }}>*</Text>
        </Text>

        <Text style={{ color: c.text.muted }}>
          This is used to personalize your profile. You can change it later.
        </Text>
      ...
      >
        ...
        <Text style={{ color: c.text.muted, lineHeight: 20 }}>
          We just need this to finish creating your account.
        </Text>
      </View>
