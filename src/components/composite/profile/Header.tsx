...
export default function ProfileHeader() {
  ...
  return (
    <View style={{ alignItems: "center", gap: 12, paddingVertical: 8 }}>
      ...
      >
        <Text
          style={{ color: c.text.secondary, fontWeight: "700", fontSize: 18 }}
        >
          {initials}
        </Text>
      </View>
      ...
      {/* Email (read-only; managed by sign-in) */}
      <TextInput
        value={email}
      ...
      <Text style={{ color: c.text.muted, fontSize: 12, marginTop: -6 }}>
      ...
      >
        {age != null && (
          <Chip label={`Age ${age}`} selected={false} onPress={() => {}} />
        )}
        {sexLabel && (
          <Chip label={sexLabel} selected={false} onPress={() => {}} />
        )}
        {raceLabel && (
          <Chip label={raceLabel} selected={false} onPress={() => {}} />
        )}
        {heightDisplay && (
          <Chip
            label={`Ht ${heightDisplay}`}
            selected={false}
            onPress={() => {}}
          />
        )}
