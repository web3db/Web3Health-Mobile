...
export async function sendSegmentSuccess(postingId: number, dayIndex: number) {
  const ok = await ensureNotifPermission();
  if (!ok) return;
  const allow = await throttleOnce(
    `segment.${postingId}.${dayIndex}`,
    60 * 1000,
  );
  if (!allow) return;
...
export async function sendOpenAppNudge(
  ...
  // Server push (cron) owns reminder nudges in production
  if (!__DEV__) return;
...
// === [ANCHOR: NOTIFY-TAP-ROUTER] notification tap listener (local + remote)
export function onNotificationResponse(
  handler: (data: Record<string, any>) => void,
) {
  const sub = Notifications.addNotificationResponseReceivedListener((resp) => {
    const data =
      (resp?.notification?.request?.content?.data as Record<string, any>) ?? {};
    handler(data);
  });
