...
function BackgroundBootWhenSignedIn() {
  ...
  React.useEffect(() => {
    ...
    if (__DEV__) console.log('[APP BOOT] ready; config=', getShareRuntimeConfig());
    return () => { (globalThis as any).__SHARE_READY__ = false; };
  }, [ready]);

  // Background tasks + notifications (lazy import AFTER ready)
  React.useEffect(() => {
    if (!ready) return;
    (async () => {
      if (Platform.OS === 'android' || Platform.OS === 'ios') {
        try {
          ...
          const { registerShareBackgroundTask } = await import('@/src/background/shareTask');
          await initNotifications();
          await registerShareBackgroundTask();
          if (__DEV__) console.log('[BG] registerShareBackgroundTask → ok');
        } catch (e: any) {
